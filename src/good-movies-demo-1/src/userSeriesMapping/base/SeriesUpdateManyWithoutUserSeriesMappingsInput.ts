/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { SeriesWhereUniqueInput } from "../../series/base/SeriesWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class SeriesUpdateManyWithoutUserSeriesMappingsInput {
  @Field(() => [SeriesWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SeriesWhereUniqueInput],
  })
  connect?: Array<SeriesWhereUniqueInput>;

  @Field(() => [SeriesWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SeriesWhereUniqueInput],
  })
  disconnect?: Array<SeriesWhereUniqueInput>;

  @Field(() => [SeriesWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [SeriesWhereUniqueInput],
  })
  set?: Array<SeriesWhereUniqueInput>;
}

export { SeriesUpdateManyWithoutUserSeriesMappingsInput as SeriesUpdateManyWithoutUserSeriesMappingsInput };
